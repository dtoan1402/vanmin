...
void SHA256string::transform(const unsigned char *message, unsigned int block_nb)
{
	uint32 w[64];
	uint32 wv[8];
	uint32 t1, t2;
...
void SHA256string::update(const unsigned char *message, unsigned int len)
{
...
void SHA256string::final(unsigned char *digest)
{
	unsigned int block_nb;
	unsigned int pm_len;
	unsigned int len_b;
...
{
	unsigned char digest[SHA256string::DIGEST_SIZE];
	memset(digest, 0, SHA256string::DIGEST_SIZE);

	SHA256string ctx = SHA256string();
	ctx.init();
	ctx.update((unsigned char*)input.c_str(), input.length());
	ctx.final(digest);

	char buf[2 * SHA256string::DIGEST_SIZE + 1];
	buf[2 * SHA256string::DIGEST_SIZE] = 0;
	for (int i = 0; i < SHA256string::DIGEST_SIZE; i++)
		sprintf(buf + i * 2, "%02x", digest[i]);
	return std::string(buf);
}
std::string RandomString(int len)
{
	static int mySeed = (rand() % 163345422624759818) + 1;
	srand(time(0) * len + ++mySeed*mySeed);
...
extern "C" char* Goblin() {
	...
	while (sha256(charstest).substr(0, 2) != "00") {
